...
Author:  Arun Sirpal 
Title:   SQL 2008/R2/2012/2014 SQL Performance Dashboards


UPDATE History:
			AUG 2015 Alpha version - Intial dev using core reports from MS
			OCT 2015 Beta Version - CPU SSRS rendering fixed and overflow fixed.
			DEC 2014 Charlie Version - Check Dependancies SP contained errors - now fixed.
			JAN 2016 Delta version - Now Supports SQL 2012+ memory stats

Comments: This is the source code for article: http://www.sqlservercentral.com/articles/SQL+Server/127992/
...
go
PRINT 'Function WaitTypeCategory created'
go
...
go
PRINT 'Function Query Handle created'
go
...
go
PRINT 'Function HEXtoVar created'
go
...
go
PRINT 'Stored Procedure Check Depencies created'
go
...
go
PRINT 'Stored Procedure CPU History created'
go
...
go
PRINT 'Stored Procedure GetMiscInfo created'
go
...
go
PRINT 'Stored Procedure GetSession created'
go
...
PRINT 'Stored Procedure GetRequestInfo created'
go
...
PRINT 'Stored Procedure RequestWaits created'
go
...
go
PRINT 'Stored Procedure PageDetails created'
go
...
PRINT 'Stored Procedure PlanGuides created'
go
...
PRINT 'Stored Procedure ShowPlanXML created'
go
...
go
PRINT 'Multiple SHOWPLAN functions created'
...
/****** Object:  StoredProcedure [MS_PerfDashboard].[usp_CheckDependencies]    Script Date: 10/08/2013 11:47:23 ******/
SET ANSI_NULLS ON
GO
...
ALTER procedure [MS_PerfDashboard].[usp_CheckDependencies]
as
begin
	declare @Version nvarchar(100)
	declare @MajorVer tinyint, @MinorVer tinyint, @BuildNum smallint
...
PRINT 'Stored Procedure Indexes created'
...
PRINT 'CPU OVERFLOW FIXES APPLIED'
--*******************************************
--Fix for 2008R2 if cpu ssrs renders overflow
--*******************************************
...
grant execute on MS_PerfDashboard.usp_Main_GetCPUHistory to public
go
PRINT 'SSRS Render fix applied'


-- FIX FOR THE SQL IN REPORT RECENTCPU.RDL FOR SESSION HISTORY ONLY (TSQL BLOCK ONLY)
/**
...
if object_id('MS_PerfDashboard.usp_GetMainLink', 'P') is not null
	drop procedure MS_PerfDashboard.usp_GetMainLink
go




create procedure MS_PerfDashboard.usp_GetMainLink
as
begin
...
PRINT 'Stored Procedure for LINKS created'
go



----------------------------------------------------
--SQL Memory stats Section runs depending on version 
----------------------------------------------------

PRINT '***********************************'
PRINT 'SQL Server Memory section CREATED'
PRINT '***********************************'
...
IF OBJECTPROPERTY(object_id('MS_PerfDashboard.usp_ram_Server'), N'IsProcedure') = 1
DROP PROCEDURE [MS_PerfDashboard].[usp_ram_Server]
GO
...
CREATE PROCEDURE [MS_PerfDashboard].[usp_ram_Server]
AS
BEGIN
-- For SQL 2008/2008R2 
-- ASIRPAL 16/02/2016
...
SELECT @Instancename = 
LEFT([object_name], (CHARINDEX('':'',[object_name]))) 
...
SELECT	
	 CEILING(physical_memory_in_bytes/1048576.0) as [Server_Memory_MB]
...
SET @DSQL  = N'CREATE PROCEDURE [MS_PerfDashboard].[usp_ram_Server]
AS
BEGIN
-- Update for SQL 2012 onwards
-- ASIRPAL 17/02/2016
...
SELECT	
	 CEILING(physical_memory_kb/1024) as [Server_Memory_MB]
		FROM sys.dm_os_sys_info 
		END
'
...
IF OBJECTPROPERTY(object_id('MS_PerfDashboard.usp_BPOOL_Commit'), N'IsProcedure') = 1
DROP PROCEDURE [MS_PerfDashboard].[usp_BPOOL_Commit]
GO
...
CREATE PROCEDURE [MS_PerfDashboard].[usp_BPOOL_Commit]
AS
BEGIN
-- For SQL 2008/2008R2 
-- ASIRPAL 16/02/2016
...
DECLARE @pg_size INT, @Instancename VARCHAR(50) 
SELECT @pg_size = low FROM master..spt_values 
...
--SELECT ''Buffer Pool commit assigned'' 
SELECT (bpool_commit_target*8)/1024 as [BPool_Commit_Tgt_MB]
FROM sys.dm_os_sys_info 
END
...
SELECT (committed_target_kb )/1024 as [BPool_Commit_Tgt_MB]
FROM sys.dm_os_sys_info 
...
IF OBJECTPROPERTY(object_id('MS_PerfDashboard.usp_BPOOL_Used'), N'IsProcedure') = 1
DROP PROCEDURE [MS_PerfDashboard].[usp_BPOOL_Used]
GO
...
CREATE PROCEDURE [MS_PerfDashboard].[usp_BPOOL_Used]
AS
BEGIN
-- For SQL 2008/2008R2 
-- ASIRPAL 16/02/2016
...
SET NOCOUNT ON
		...
		--SELECT ''Memory currently used by Buffer Pool'' 
		SELECT cntr_value/1024 as [BPool_In_Use_MB]
		FROM #perfmon_counters 
...
IF OBJECTPROPERTY(object_id('MS_PerfDashboard.usp_PLE'), N'IsProcedure') = 1
DROP PROCEDURE [MS_PerfDashboard].[usp_PLE]
GO


DECLARE @DSQL3 NVARCHAR(MAX)
...
CREATE PROCEDURE [MS_PerfDashboard].[usp_PLE]
AS
BEGIN
-- For SQL 2008/2008R2 
-- ASIRPAL 16/02/2016
...
SELECT cntr_value as [PLE_sec],CASE WHEN (cntr_value > 400) 
											THEN ''PLE is Healthy'' 
											ELSE ''PLE requires investigation''
											END as ''PLE_Status''  
FROM #perfmon_counters WHERE object_name=@Instancename+''Buffer Manager'' 
